...
export function corsHeaders(origin) {
  return {
     ...
     *
     * x-fs-build was in the expose list above but was only ever set on the
     * /api/chat POST, so a preflight read it as null — prompt-check printed
     * "build (unknown)" and the run was unattributable to a build. Staleness is
     * this project's recurring failure mode; a stamp that is absent from the
     * cheapest request you can make is a stamp that will be missing exactly
     * when someone is trying to work out which code produced a result.
     */
